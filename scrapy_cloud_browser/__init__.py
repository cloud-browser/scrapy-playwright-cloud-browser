...
from typing import Awaitable, Callable, cast, Literal, Optional, Union
...
from pydantic import AnyUrl, BaseModel, conlist, constr, HttpUrl, PositiveInt
...
log = logging.getLogger('browser_handler')
...
class BrowserContextWrapper:
    ...
    async def close(self):
        log.warn(f'{self.num}: Close browser')
        if self.context:
            try:
...
CURRENT_WRAPPER = ContextVar('current_wrapper', default=None)
...
class CloudBrowserHandler(ScrapyPlaywrightDownloadHandler):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.crawler: Crawler = cast(Crawler, args[0])
